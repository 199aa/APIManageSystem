...
public class AlertCheckTask {
  ...
  public void checkAlertRules() {
    ...
    try {
      ...
      for (AlertRule rule : rules) {
        try {
          System.out.println(
              "[告警检查] 正在检查规则: " + rule.getName() + " (类型: " + rule.getType() + ", 条件: " + rule.getCondition() + ")");
  ...
  private boolean checkErrorRate(double threshold) {
    try {
      ...
      System.out.println("[告警检查] 错误率检查 - 总调用: " + totalCalls + ", 成功: " + successCalls + ", 错误率: "
          + String.format("%.2f", errorRate) + "%, 阈值: " + threshold + "%");
  ...
  private boolean checkSuccessRate(double threshold) {
    try {
      ...
      System.out.println("[告警检查] 成功率检查 - 总调用: " + totalCalls + ", 成功: " + successCalls + ", 成功率: "
          + String.format("%.2f", successRate) + "%, 阈值: " + threshold + "%");
