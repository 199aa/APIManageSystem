...
SELECT CONCAT('api_call_log 剩余记录数: ', COUNT(*)) as result FROM api_call_log;
SELECT CONCAT('operation_log 剩余记录数: ', COUNT(*)) as result FROM operation_log;

-- 4. 查看最新的记录（确认清理效果）
SELECT '=== api_call_log 最新10条记录 ===' as info;
SELECT * FROM api_call_log ORDER BY call_time DESC LIMIT 10;
